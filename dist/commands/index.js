...
exports.commandsHandler = exports.setupCommands = exports.createCommand = exports.commands = void 0;
...
const pickColor = () => (0, utils_1.pick)([
    '#de2316',
    '#f09841',
    '#e0d316',
    '#ace334',
...
const getGameCategoryAndCatalog = (interaction) => {
    ...
    if (!guild)
        return { catalogChannel: undefined, gamesCategory: undefined };
...
const setupCommands = async (guild) => {
    await (0, exports.createCommand)(guild, {
        name: 'generate-relationship',
        description: 'generates a relationship for you',
        execute: (interaction) => {
            interaction.reply((0, relationshipGenerator_1.generateRelationship)(guild));
        }
    });
    await (0, exports.createCommand)(guild, {
        name: 'add-game',
        ...
        execute: async (interaction) => {
            ...
            const channelName = interaction.options.get('channelname')?.value;
            ...
            if (!(catalogChannel && gamesCategory)) {
                return interaction.reply(`Cannot make the game, make sure that this server has a category that `
            ...
            if (!catalogMessage) {
                catalogMessage = await catalogChannel.send(`This is the catalog of games we have in this server, react with an emoji to be added to the role for that game:`);
            }
            try {
            ...
            await catalogMessage.edit(`${catalogMessage.content}`
    ...
    await (0, exports.createCommand)(guild, {
        ...
        execute: async (interaction) => {
            ...
            if (!guild)
                return;
            const gameChannel = interaction.options.get('gamechannel')?.channel;
            const { catalogChannel, gamesCategory } = getGameCategoryAndCatalog(interaction);
            if (!(catalogChannel && gamesCategory)) {
                return interaction.reply(`Cannot remove the game, make sure that this server has a category that `
                    + `includes the word "game" and a channel that includes the word "catalog"`);
            }
            const catalogMessages = await catalogChannel.messages.fetch();
            let catalogMessage = catalogMessages.find((message) => message.author.bot && message.content.includes('catalog'));
            ...
            catalogMessage.edit(catalogMessage.content
                .split('\n')
    ...
    await (0, morningMessageHandler_1.setupMorningMessageCommands)(guild, exports.createCommand);
};
exports.setupCommands = setupCommands;
const commandsHandler = (interaction) => {
    if (!interaction.isCommand())
        return;
    if (exports.commands[interaction.commandName])
        exports.commands[interaction.commandName](interaction);
};
