...
client.once('ready', async () => {
  try {
    ...
    const replyHandler = async (message) => {
      try {
        ...
        if (reference) {
          ...
          if (message.content.toLowerCase() === '-allhearts') {
            message.delete();

            const hearts = await getHearts(guild);
            _.shuffle(hearts).slice(0, 20 - reference.reactions.cache.size).forEach(async (heart) => {
              try {
                await reference.react(heart);
              } catch (e) {
                console.error(e);
              }
            });
    ...
    const generateRelationshipMessage = (message) => {
      message.reply({
        content: generateRelationship(guild),
        components: [
          new MessageActionRow({
            components: [
              createButton({
                label: 'Another!',
                style: 'PRIMARY',
                customId: 'generate-relationship',
                onPress: (interaction) => {
                  generateRelationshipMessage(interaction);
                }
              })
            ]
          })
      ...
      try {
        if (!message.author.bot) {
          ...
          if (get(message, 'channel.type') === 'DM') {
            ...
            if (message.content === '-genR') {
              generateRelationshipMessage(message);
            }
