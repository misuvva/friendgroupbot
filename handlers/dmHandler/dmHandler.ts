...
import { namesToHandles, generateAnonId } from './utils';
import { getStore, addToStore } from '../../storeUtils/storeUtils';
import { errorHandler } from '../errorHandler/errorHandler';

const anonCodesPath = 'anonCodes';

const storeAnonCode = async (code: string, userId: string) => {
  await addToStore(`${anonCodesPath}.${code}`, { userId, updatedAt: moment().toISOString() });
};
...
export const dmHandler = async (message: Message, guild: Guild) => {
  try {
    const bigGeneral = guild.channels.cache.find((channel) => channel.name.includes('big-general')) as TextChannel;

    const getName = (member: GuildMember) => member.nickname!.slice(2, -2)
      .toLowerCase()
      .split(' ')
      .join('');
    ...
    if (!senderName) return; // Should probably handle this case
      ...
      case 'say':
        {
          ...
          if (words[1][0] === '#') {
            ...
            const channel = guild.channels.cache.find((channel) => channel.name.includes(words[1].toLowerCase().slice(1))) as TextChannel;
