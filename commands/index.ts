...
import { Guild, CommandInteraction, CacheType, TextChannel, CategoryChannel, Message } from 'discord.js';
import { generateRelationship } from '../generators/relationshipGenerator';
import { setupMorningMessageCommands } from '../handlers/morningMessageHandler/morningMessageHandler';
...
export const commands: Record<string, (interaction: CommandInteraction<CacheType>) => void> = {};

const isEmoji = (string: string) => /\p{Emoji}/u.test(string);
...
export const createCommand = async (guild: Guild, { name, execute, ...commandArgs }: CommandArgs) => {
  try {
...
const getGameCategoryAndCatalog = (interaction: CommandInteraction<CacheType>) => {
  const { guild } = interaction;
  if (!guild) return { catalogChannel: undefined, gamesCategory: undefined };
  const catalogChannel = guild.channels.cache.find((channel) => channel.name.includes('catalog')) as TextChannel;
  const gamesCategory = guild.channels.cache.find((channel) => channel.name.includes('games') && channel.type === 'GUILD_CATEGORY') as CategoryChannel;

  return { catalogChannel, gamesCategory };
};

export const setupCommands = async (guild: Guild) => {
  await createCommand(guild, {
    name: 'generate-relationship',
    description: 'generates a relationship for you',
    execute: (interaction) => {
      interaction.reply(generateRelationship(guild));
    }
  });
  await createCommand(guild, {
    name: 'add-game',
    ...
    execute: async (interaction) => {
      ...
      const roleName = interaction.options.get('rolename')?.value as string;
      ...
      const gameRole = await guild.roles.create({ name: roleName, color: pickColor() as any });
  ...
  await createCommand(guild, {
    ...
    execute: async (interaction) => {
      const { guild } = interaction;
      if (!guild) return;
      const gameChannel = interaction.options.get('gamechannel')?.channel as TextChannel;
      const { catalogChannel, gamesCategory } = getGameCategoryAndCatalog(interaction);
      if (!(catalogChannel && gamesCategory)) {
        return interaction.reply(
      ...
      if (!gameLine) {
        return interaction.reply({ content: 'Could not find game in catalog', ephemeral: true });
      }
...
export const commandsHandler = (interaction: CommandInteraction<CacheType>) => {
